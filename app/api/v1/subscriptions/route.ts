...
import { createRazorpayBillingPlan } from "@/lib/razorpaySubscriptions";
...
export async function GET() {
  if (!isDatabaseConfigured()) {
    return NextResponse.json({ error: "Database not configured", subscriptions: [] }, { status: 503 });
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!product) {
      ...
      if (!fallback) {
        return NextResponse.json({ error: "Milk product not found. Run npm run db:seed" }, { status: 404 });
      }
    }
    ...
    if (data.frequency === "CUSTOM" && customDays.length === 0) {
      return NextResponse.json({ error: "Select at least one custom delivery day" }, { status: 400 });
    }
