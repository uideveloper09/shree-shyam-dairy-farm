...
export async function POST(request) {
  ...
  try {
    ...
    const { amount, items, note, coupon, method = "upi" } = body;
    ...
    order = await razorpay.orders.create({
      ...
      notes: {
        items: JSON.stringify(
          items.map((i) => ({ id: i.id, name: i.name, qty: i.quantity, price: i.price }))
        ),
        note: note || "",
        coupon: coupon?.code || "",
        pay_method: method,
    ...
    try {
      ...
      try {
        ...
        const linkMessage =
          linkErr?.error?.description || linkErr?.error?.reason || linkErr?.message || "Unknown error";
    ...
    const message = err?.error?.description || err?.error?.reason || err?.message || "Unknown error";
    ...
    return NextResponse.json({ error: "Failed to create payment QR", detail: message }, { status: 500 });
  }
}
