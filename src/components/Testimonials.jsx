...
import { CONTAINER, SECTION_CREAM, SECTION_HEAD_COMPACT } from "@/constants/layout";
  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <FaStar key={i} size={size} className={i < rating ? "text-[#C89B3C]" : "text-gray-200"} />
      ))}
    </div>
  );
}
...
function TestimonialTab({ testimonial, active, onClick }) {
  return (
    ...
    >
      {active && <span className="absolute inset-x-0 top-0 h-0.5 bg-[#C89B3C]" aria-hidden />}
      <div
