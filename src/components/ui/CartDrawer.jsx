...
${orderNote ? `\nNote: ${orderNote}` : ""}`;
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 z-[60] bg-[#061E3D]/60 backdrop-blur-[2px]"
        onClick={closeCart}
        aria-hidden
      />
        ...
        <div className="relative shrink-0 border-b border-[#eee] bg-white px-5 py-4">
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h2 className="font-heading text-[20px] font-bold text-[#082F63]">
                {cartConfig.title}
              </h2>
                ...
                <div className="border-t px-5 py-4">
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    {suggested.map((product) => (
                      <SuggestedProduct key={product.id} product={product} onAdd={addToCart} />
                    ))}
                  </div>
                </div>
              )}
