...
export function buildFooterProductLinks(products, categories = []) {
  const categoryHrefs = Object.fromEntries(categories.map((c) => [c.slug, c.href]));

  return products
    .filter((p) => p.category !== "combo")
    .map((p) => ({
      label: p.name,
      href: categoryHrefs[p.category] ?? "#products",
    }));
}
