...
import SubscribeMilkForm from "@/components/subscription/SubscribeMilkForm";
      ...
      {dbError && (
        <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          ...
          <p className="mt-1 text-xs text-amber-800">
            Add <code className="rounded bg-white/80 px-1">DATABASE_URL</code> to .env.local, then run{" "}
            <code className="rounded bg-white/80 px-1">npm run db:push</code> and{" "}
      ...
      <div className="mt-6 space-y-4">
        {subscriptions.length === 0 && !showForm && !dbError && (
          <div className="rounded-2xl border border-dashed border-[#C89B3C]/40 bg-[#faf9f6] py-12 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">Subscribe for farm-fresh milk at your doorstep</p>
            <button
              type="button"
              onClick={() => setShowForm(true)}
