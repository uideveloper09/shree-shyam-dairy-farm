...
export function buildKnowledgeBase(content) {
  ...
  const coupons =
    cart?.coupons?.map((c) => `${c.code} (${c.label})`).join(", ") || "FRESH10";
...
export function getLocalReply(userMessage, content) {
  ...
  if (includesAny(msg, ["product", "price", "rate", "cost", "kitne", "doodh", "milk", "ghee", "paneer", "dahi", "curd", "chaach", "chach", "combo"])) {
    const list = products
      .filter((p) => p.inStock !== false)
      .slice(0, 8)
  ...
  if (includesAny(msg, ["order", "buy", "purchase", "checkout", "cart", "khareed", "kaise order"])) {
  ...
  if (includesAny(msg, ["farm", "gaay", "cow", "about", "story", "quality", "fresh", "pure", "shuddh"])) {
